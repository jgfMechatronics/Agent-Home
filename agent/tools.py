...
Memory tools return error strings on failure (for model self-correction),
not exceptions.
"""
...
def _resolve_occurrence(
    ...
    """Resolve target string to a character index, or return error string.
    ...
    Returns:
        Start character index on success, or error string on failure.
    """
    indices = _find_occurrences(content, target)
    
    if not indices:
        return f"Error: '{target}' not found in block '{label}'"
    
    if len(indices) > 1 and occurrence is None:
        return f"Error: '{target}' appears {len(indices)} times. Specify occurrence (1-{len(indices)})."
    
    # Convert to 0-indexed
    target_idx = 0 if occurrence is None else occurrence - 1
    
    if target_idx < 0:
        return "Error: occurrence must be >= 1 (1-indexed)"
    
    if target_idx >= len(indices):
        return f"Error: occurrence {occurrence} not found (only {len(indices)} occurrences exist)"
...
async def memory_replace(
    ...
    Returns:
        Snippet of updated content on success, or error message on failure.
    """
    deps = ctx.deps
    ...
    # Resolve occurrence to character position
    result = _resolve_occurrence(block.content, old_string, occurrence, label)
    ...
    if len(new_content) > block.char_limit:
        return f"Error: result would exceed char_limit ({len(new_content)} > {block.char_limit})"
...
async def memory_insert(
    ...
    if not after:
        return "Error: 'after' cannot be empty. Use '<start>' or '<end>' for boundaries."
    ...
    if after in ("<start>", "<end>"):
        if occurrence is not None:
            return "Error: occurrence cannot be used with '<start>' or '<end>'"
        insert_pos = 0 if after == "<start>" else len(block.content)
    else:
        # Resolve anchor occurrence to character position
        result = _resolve_occurrence(block.content, after, occurrence, label)
        if isinstance(result, str):
            return result  # Propagate error
        # Insert AFTER the anchor
        insert_pos = result + len(after)
